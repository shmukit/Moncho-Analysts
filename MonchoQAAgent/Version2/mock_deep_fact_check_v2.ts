...
// and verifies real behavior — no live API keys, no subprocess (so the
// mocked fetch is actually seen by the code under test).
//
// Run: npx tsx test/mock_deep_fact_check_v2.ts
    ...
    return {
      ...
      json: async () => ({
        results: isAnthropicFundingQuery
          ...
          : [], // unrelated/vague queries genuinely return nothing relevant
      }),
    } as any;
  }
  if (typeof url === "string" && url.includes("anthropic.com")) {
    ...
    const verdict = claimIsFunding
      ? {
          verdict: "supported",
          confidence: 0.92,
          evidence: [{ url: "https://www.reuters.com/example-anthropic-funding", note: "Confirms $13B Series F, March 2025." }],
...
async function run() {
  ...
  const verdict2 = await mod.judgeClaim("TotallyRealCompanyXYZ", "Good market position with great potential.", evidence2);
