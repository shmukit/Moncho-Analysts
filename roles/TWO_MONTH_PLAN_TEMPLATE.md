# Two-Month Plan — [Your Name] · [Data Ops | GTM Ops]

**Submit to founder for approval before full execution.**

---

## 1. Coverage snapshot summary

*What Moncho has today across **all 10 grant sectors** — see [`GRANT_TEN_SECTORS.md`](GRANT_TEN_SECTORS.md).*

### Sector coverage (fill one row per sector)

| # | Sector | Depth (🟢/🟡/🔴) | Your focus in ~2 months |
|---|--------|-------------------|-------------------------|
| 1 | Finance / Banking | | |
| 2 | Agriculture | | |
| 3 | Education | | |
| 4 | ICT / ITES | | |
| 5 | Energy | | |
| 6 | Health | | |
| 7 | Logistics | | |
| 8 | E-commerce | | |
| 9 | Sports | | |
| 10 | RMG / Textiles | | |

Brief notes (facts, gaps, blockers):

-

---

## 2. Outcomes (max 5)

Measurable outcomes, not task lists.

| # | Outcome | Metric | Target |
|---|---------|--------|--------|
| 1 | | | |
| 2 | | | |
| 3 | | | |
| 4 | | | |
| 5 | | | |

---

## 3. Milestones (ordered steps)

| Step | Milestone | Done when |
|------|-----------|-----------|
| 1 | | |
| 2 | | |
| 3 | | |
| 4 | | |
| 5 | | |
| 6 | | |

---

## 4. Grant checkpoint — your Sep report section

Which [`PROGRESS_REPORT_TEMPLATE`](https://github.com/shmukit/Moncho-V1/blob/main/docs/07-projects/bangladesh-ict-division-research-grant/PROGRESS_REPORT_TEMPLATE.md) sections will you draft?

-

---

## 5. Handoff to founder (JSON/SQL)

| Artifact type | Sector | Status |
|---------------|--------|--------|
| | | draft / submitted / merged |

---

## 6. Dependencies and risks

| Dependency / risk | Mitigation |
|-------------------|------------|
| | |

---

## 7. Founder approval

- [ ] Plan reviewed
- [ ] Scope agreed

**Sign-off:** _______________ **Date:** _______________
