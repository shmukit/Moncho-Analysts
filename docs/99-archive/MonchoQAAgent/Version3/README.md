...
## Landscape/Expert schemas — now exact

`validateLandscapeSchema()` and `validateExpertSchema()` in `qa_reviewer.ts`
now match your real `landscape_sample.json` / `expert_sample.json` shapes
exactly (ID-based, not slug-based — `sector_id`, `segment_ids: number[]`,
`country_id` as ISO-2, `tam_data` as a nested object, orgs-within-segments
referenced by UUID). Notably:

- **Never guess IDs.** Pass `--valid-sector-ids ids.json` and
  `--valid-segment-ids ids.json` (each a flat JSON array of integers,
  e.g. `[5, 12, 301]`) and the checker will flag any `sector_id` /
  `segment_id` / `segment_ids` value not in that reference list — directly
  enforcing your own "don't guess values like 2 or 201" rule.
- **UUID check on landscape organizations.** `segments[].organizations[].id`
  must be a real UUID (an org already created and approved in your DB) —
  not a name-derived placeholder. Tested against your own sample's literal
  `"uuid-v1"` / `"uuid-v2"` values, which correctly FAIL this check — a
  good reminder that those are illustrative shorthand in the sample doc,
  not a format to actually submit.
- `status` and `classification` are checked against a conservative default
  enum (`draft/published/archived/in_review`, `public/private/internal`).
  If your real enums differ, tell me and I'll update the constants — these
  are soft assumptions where the schema doc didn't give an exhaustive list.
