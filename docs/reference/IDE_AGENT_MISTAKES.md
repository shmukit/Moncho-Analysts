...
| A-03 | **Invent sector-specific “approved” rubrics** — write ICT/agri/energy scoring or a new `template_id` as if production | Analysts follow unapproved rules; rework after founder review | Use the harvest card in `PRODUCT_ORG_RUBRICS.md` Part 3c (`diagnostics-test.v1`, `solar-hardware.v1`, …). Propose a **new** template only in the sector plan for founder sign-off. Do not treat hypotheses as law. |
...
| P-08 | **Sports club revenue as physical SKUs only** — skip tickets/sponsorship or force HS codes | Empty sports pricing; wrong segment mapping | Submit service lines as products + `product_metrics` with `variant_unit` (`ticket`, `month`, `year`). `metadata.revenue_stream` + `b2b: true` on sponsorship/broadcast. Sample: `samples/sports_revenue_sample.json`. |
| P-09 | **Single price field for B2B ranges** — `"85000-180000"` or CMS truncates to first number | Rejected apply RPC or silent data corruption | Two rows per band (low/high), shared `metadata.band_id`. See `PRODUCT_ORG_RUBRICS.md` § Part 3b. |
| P-10 | **B2B sponsorship as market-sizing price** — ৳50L jersey deal in TAM formula | Guardrail reject or absurd TAM | Catalog-only for B2B; sizing input = consumer lines (ticket, academy, membership) only. |
| P-11 | **Treat every accepted product row as a priced SKU** — ignore `pricing_gap: true` or rubric-only harvest packets | Reviewers approve intelligence rows; agents expect Pricing tab rows that were never submitted | Check shape before backfill or chart debugging. Priced: CMS `brand_name`+`price`, harvest `price_bdt`, or `products[]` with `price_bdt`. Not priced: `pricing_gap`, rubric dims only. Catalog apply still writes a **seller-link** `product_metrics` row with **NULL price** so the org page can list the offering; Pricing APIs must skip null/non-positive prices. Intern checklist: `SKU_PRICING_SUBMISSION_GUIDE.md`. Founder audit: `npm run product-pricing:audit-submissions` (platform repo). |
...
| P-16 | **Write product copy for analysts** — “listed by…”, “named item in official local category”, SPA/HTTP fetch notes, “captured… revalidate”, Dashboard/media plumbing in `product_description` / rationales / `price_note` / `_comment` | Moncho visitors see workbench jargon instead of shopper-facing product language | Phrase Dashboard-facing fields for **website visitor customers**. Put harvest limits only in the review MD. Keep `_comment` as a short non-process stub if mechanical QA requires it. Sector lock: `Shudipta/products/Toys/WHAT_NOT_TO_DO.md` (same spirit as `Shudipta/FIs/WHAT_NOT_TO_DO.md`). |
| P-17 | **Treat `samples/product_sample.json` as the complete production product schema** — emit only name/group/category/description | Harvest JSON loses org linkage, sources, taxonomy, pricing gaps, quality dims/rationales, totals, and media provenance | Use the sample as the required base, then port approved sector template fields from Phase 1B precedents (org/source, slugs, pricing, scores, media). Keep evidence in JSON, not only in review prose. |
