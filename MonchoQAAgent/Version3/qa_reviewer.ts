...
function detectType(record: any): "organization" | "product" | "landscape" | "expert" | "unknown" {
  ...
  if (record.version_name !== undefined || (Array.isArray(record.segments) && record.tam_data !== undefined)) return "landscape";
  if (record.linkedin_url !== undefined || record.country_id !== undefined || record.segment_ids !== undefined) return "expert";
...
async function main() {
  ...
  records.forEach((record, idx) => {
    ...
    const schemaErrors =
      ...
      recordType === "landscape" ? validateLandscapeSchema(record, validSectorIds, validSegmentIds) :
      recordType === "expert" ? validateExpertSchema(record, validSegmentIds) :
